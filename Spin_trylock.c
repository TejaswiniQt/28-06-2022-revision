...
int my_thread_fun1(void *p)
{
		...
		else if(spin_trylock(&char_spinlock)==0)
		{
		printk(KERN_ERR "Didn't get a lock\n");
		}
		else
		{
		printk(KERN_ERR "Lock is in busy state\n");
		}
		}
	return 0;
}
...
int my_thread_fun2(void *p)
{
      while(!kthread_should_stop())
	{
		spin_lock(&char_spinlock);   // step 4 for mutex 
		my_global_variable++;
		printk(KERN_INFO "my_thread_handle1 =  %lu\n", my_global_variable);
		spin_unlock(&char_spinlock);  // step 5 for mutex 
		msleep(1000);	
	}
	return 0;
}
